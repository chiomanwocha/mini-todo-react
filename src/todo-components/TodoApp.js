    ...
    <div>
      {status === "loading" ? (
        ...
        <div className="todo-app">
          ...
          <h1>todos</h1>
          {todoError && (
            <p className="error">{todoError?.response.data.message}</p>
          )}
          {doingError && (
            <p className="error">{doingError?.response.data.message}</p>
          )}
          {(todoLoading || doingLoading || doneLoading || updating) && (
            <p className="alert">Updating todos...</p>
          )}
          {isDeleting ? (
          ...
          <div className="todo-list">
            <div>
              <h2>todo</h2>
              {saveError && (
                <p className="error">{saveError?.response.data.message}</p>
              )}
              <>
                <ul className="todo">
                  {data?.data?.data
                        ...
                        {id !== todo.id ? (
                          <div className="not-editing">
                            <input
                              ...
                              name="done"
                              id="done"
                              onClick={() => addDoing(todo.id)}
                            />
                            <p>{todo.title}</p>
                            <div className="todo-buttons">
                              <button onClick={() => setId(todo.id)}>
                                {" "}
                                Edit
                              </button>
            ...
            <div>
              ...
              <ul className="doing">
                {data?.data?.data
                  ?.filter((data) => data.status === "doing")
                  .map((todo) => (
                    <li className="doing-box" key={todo.id}>
                      <p>{todo.title}</p>
                      <div className="doing-button">
                        <button onClick={() => revertDoing(todo.id)}>
                          Undo
                        </button>
              ...
              {showCardDoing ? (
                <form onSubmit={(e) => doingAdd(e)}>
                  ...
                  <button
                    className="close-button"
                    onClick={() => setShowCardDoing(false)}
                  >
                    X
                  </button>
                </form>
